...
void *main_serve(void* id)
{
   // uint32_t NewId = *(uint32_t *) id;
   ARGV* NewId = (ARGV*)id;
   printf("Funcionou, meu Id = %u \n",NewId->id);
...
int main(int argc, char *argv[])
{
     ...
     */
		...
		ARGV server_argv[nservers];

   for(uint32_t i = 0; i < nservers; i++)
    {
		server_argv[i].id = i;
		server_argv[i].niter = niter;
    	thread_create(&server_thread[i],NULL,main_serve, &server_argv[i]);
    	printf("Creating: \n");
     }
     ...
     */
		 pthread_t client_thread[nclients]; // id do clinet
		 ARGV argv_client[nclients]; // argumento do client;
		 for(uint32_t i = 0; i < nclients; i++){
			argv_client[i].id = i;
			argv_client[i].niter = niter;
			thread_create(&client_thread[i], NULL,main_client,&argv_client[i]);	
     }
     ...
     */
      for(uint32_t i = 0; i < nservers; i++)
	  {
    	thread_cancel(server_thread[i]);
		printf("Servers thread %d has terminated\n", i);
      }
