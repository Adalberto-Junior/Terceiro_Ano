...
int main(int argc, char *argv[])
{
    ...
    fifo::destroy();
      ...
      void callService( fifo::serviceRequest char *req, fifo:serviceResponse char *res)
    {
      uint32_t id = fifo ::getFreeBuffer ();/* take a buffer out of fifo of free buffers */
      fifo::putRequestData(req,id); /* put request data on buffer */
      fifo::addNewPendingRequest(id); /* add buffer to fifo of pending requests */
      fifo::waitForResponse(id); /* wait (blocked) until a response is available */  
      resp =  fifo::getResponseData(id);/* take response out of buffer */
      fifo::releaseBuffer(id); /* buffer is free, so add it to fifo of free buffers */

    }
void processService()
{
	uint32_t id = fifo::getPendingRequest(); /* take a buffer out of fifo of pending requests */
	char req[fifo::Max_Str_len + 1]
	req[fifo::Max_Str_len] = '\0';
	req = fifo::getRequestData(id); /* take the request */
	fifo::Response resp;
	...
	fifo::putResponseData(&resp,id);/* put response data on buffer */
	fifo::wakeupClient(id); /* so client is waked up */
}
