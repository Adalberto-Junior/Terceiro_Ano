...
typedef struct
{
   ...
   int done=0; // 0: waiting for consultation; 1: consultation finished
   pthread_mutex_t fpatient;
   pthread_cond_t notfy;
} Patient;
...
void init_simulation(uint32_t np)
{
   ...
   for(uint32_t i = 0; i < MAX_PATIENTS; i++){
   	mutex_init(&hd->all_patients[i].fpatient,NULL);
	cond_init(&hd->all_patients[i].notfy,NULL);
   }
}
...
void doctor_iteration()
{
  ...
  // printf("\e[32;01mDoctor: patient %u treated\e[0m\n", patient);
   ...
   mutex_lock(&hd->all_patients[patient].fpatient);
   
   hd->all_patients[patient].done = 1;
   printf("\e[32;01mDoctor: patient %u treated\e[0m\n", patient);
   cond_broadcast(&hd->all_patients[patient].notfy);

   mutex_unlock(&hd->all_patients[patient].fpatient);
}
...
void patient_wait_end_of_consultation(int id)
{
   ...
   mutex_lock(&hd->all_patients[id].fpatient);

   while(hd->all_patients[id].done != 1 ){ //emquanto não for o done, faz o wait
        cond_wait(&hd->all_patients[id].notfy, &hd->all_patients[id].fpatient);
   }
   mutex_unlock(&hd->all_patients[id].fpatient);
   check_valid_name(hd->all_patients[id].name);
...
int main(int argc, char *argv[])
{
   ...
   /*lanch patiente*/
   pthread_t patier[npatients];  //duvida
  int p[npatients];
   for(int i = 0; i < npatients; i++){
	p[i] = i;
	thread_create(&patier[i],NULL,&patient_thread,&p[i]); 		
   }

   //Lnach nurse:
   pthread_t nurse[nnurses];
   for(int i = 0; i < nnurses; i++){
  	 thread_create(&nurse[i],NULL,&nurse_thread,NULL);
   }
   
   //lanch doctor:
   pthread_t doctor[ndoctors];
   for(int i = 0; i < ndoctors; i++){
         thread_create(&doctor[i],NULL,&doctor_thread,NULL);
   }
   /*
   //finish nurse_thread
   for(uint32_t i = 0; i < nnurses; i++){
        thread_join(nurse[i],NULL);
       // printf("paciente %d terminou\n",i);
   }
     //finish doctor_thread
   for(uint32_t i = 0; i < ndoctors; i++){
        thread_join(doctor[i],NULL);
       // printf("paciente %d terminou\n",i);
   }
   */
//finish patient_thread
   printf("Antes do Join\n ");
   for(uint32_t i = 0; i < npatients; i++){
	printf("paciente %d terminou\n",i);
   	thread_join(patier[i],NULL);
   }
...
void new_patient(Patient* patient)
{
   ...
   patient->done = 0;
  // mutex_init(&patient->fpatient,NULL);
   //cond_init(&patient -> notfy,NULL);
}
